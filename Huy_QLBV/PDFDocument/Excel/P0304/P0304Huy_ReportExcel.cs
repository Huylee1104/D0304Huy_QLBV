...
{
    private List<M0304Huy_Mau4> _data;
    ...
    public P0304ExcelReportTemplate(List<M0304Huy_Mau4> data, List<M0304ThongTinDoanhNghiep> dataDN, string ngayBatDau, string ngayKetThuc,
    ...
    public byte[] GenerateExcel()
    {
        using (var wb = new XLWorkbook())
        {
            ...
            // Nếu muốn thông tin nằm ngang logo (cột 4–10, hàng 1–3)
            ...
            // Tiêu đề chính, căn giữa, font lớn, bold
            ...
            {
                int stt = 1; // STT chạy liên tục cho toàn báo cáo
                foreach (var nv in _danhSachNhanVien)
                {
                    ...
                    // Dòng tên nhân viên + tổng cá nhân
                    ...
                    // Kẻ border cho toàn bộ hàng (cột 1 → 10)
            ...
            {
                // Trường hợp 1 nhân viên: dùng _tenNhanVien và _tongHuy/_tongHoan/_tongSoTien
                ws.Range(currentRow, 1, currentRow, 7).Merge();
                ws.Cell(currentRow, 1).Value = $"Tên nhân viên: {_tenNhanVien}";
                ws.Cell(currentRow, 1).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Left; // căn trái
                ws.Cell(currentRow, 1).Style.Font.Bold = true;
                ws.Cell(currentRow, 1).Style.Font.FontSize = 10;
            ...
            // Đặt viền cho toàn bộ vùng merged, cả bên ngoài và bên trong
            ...
            // ========== PHẦN CUỐI (Số tiền phải nộp, bằng chữ, ngày ký, người lập) ==========
            // Dòng "Số tiền phải nộp"
            ws.Range(currentRow, 1, currentRow, 6).Merge(); // merge từ cột 1 đến cột 7
            ...
            // Merge từ cột 7 đến cột 10, cao 4 hàng
            ws.Range(currentRow, 7, currentRow + 5, 10).Merge();
            // Nội dung trong ô đã merge
